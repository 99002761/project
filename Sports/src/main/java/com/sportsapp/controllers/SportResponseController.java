...
public class SportResponseController {
	...
	SportService sportService;
	  ...
	  @GetMapping("/sports/stadiumName/{stadiumName}")
	    ...
	    @GetMapping("/books/sportGame/{sportGame}")
	    ResponseEntity<List<Sports>> getBySportGame(@PathVariable("sportGame")String sportGame){
	        List<Sports> sportList = sportService.getBySportGame(sportGame);    
	        return ResponseEntity.ok(sportList);
	    }
	    @GetMapping("/sports/get-one/{sportsId}")
	    ResponseEntity<Sports> findSportById(@PathVariable("sportsId")Integer sportsId) throws SportNotFoundException {
	        Sports nsport = sportService.findSportById(sportsId);
	        HttpHeaders header = new HttpHeaders();
